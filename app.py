...
# --- Helper Functions ---
@st.cache_data(ttl=60) # Cache for 1 minute to ensure fresh data around allocation time
...
def get_target_date():
    now = get_kst_time()
    # 자정(00:00) 마감 시스템: 현재 시각과 무관하게 신청은 '다음 주차 가능일'을 향함
...
if requests_data["target_date"] != str(target_date):
    ...
    # 🚨 CRITICAL FIX: Race Condition Safety
    # 자정에 날짜가 바뀌었을 때, auto_allocate.py가 아직 배정을 끝내지 않은 상태에서
    # 사용자가 접속해 requests_data를 비워버리면 배정 정보가 통째로 날아갑니다.
    if old_date:
        history_old = next((h for h in history if h["date"] == old_date), None)
        # 만약 이전 날짜가 히스토리에 없거나 slack_notified가 False라면, 아직 배정 중인 것임!
        if not history_old or not history_old.get("slack_notified", False):
            st.warning(f"🔄 **{old_date}** 자동 주차 배정 작업이 현재 진행 중입니다. 약 1~2분 뒤에 새로고침 한 후 신청해주세요.", icon="⏳")
            st.stop()
...
if st.session_state.page == "main":
    ...
    with col_t1:
        ...
        # 24:00 마감 / 24:01 발표 체제이므로 별도의 하드 마감 시간(오전 8시) 블로킹 제거
